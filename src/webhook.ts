...
app.post('/webhook', (req, res) => {
  ...
  if (req.body.ref === "refs/heads/main") {
    ...
    const commands = [
      ...
      `${dockerComposePath} stop backend db phpmyadmin nginx && ${dockerComposePath} rm -f backend db phpmyadmin nginx`,  // 먼저 중지하고 제거
      'sleep 10',  // 10초 대기
      `${dockerComposePath} up -d --build backend db phpmyadmin nginx`  // 나머지 서비스 시작
    ];
